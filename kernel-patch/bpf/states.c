	...
	list_for_each_safe(pos, tmp, head) {
		...
		if (sl->state.branches) {
			...
			if (is_iter_next_insn(env, insn_idx)) {
				if (BPF_PROFILE_CALL_ARG(BPF_PROFILE_STATES_FILE_ID,insn_idx, states_equal, env, &sl->state, cur, RANGE_WITHIN)) {
			...
			if (is_may_goto_insn_at(env, insn_idx)) {
				if (sl->state.may_goto_depth != cur->may_goto_depth &&
				    BPF_PROFILE_CALL_ARG(BPF_PROFILE_STATES_FILE_ID, insn_idx, states_equal, env, &sl->state, cur, RANGE_WITHIN)) {
					loop = true;
					goto hit;
				}
			}
			...
			if (states_maybe_looping(&sl->state, cur) &&
			    BPF_PROFILE_CALL_ARG(BPF_PROFILE_STATES_FILE_ID, insn_idx, states_equal, env, &sl->state, cur, EXACT) &&
...
skip_inf_loop_check:
		...
		if (BPF_PROFILE_CALL_ARG(BPF_PROFILE_STATES_FILE_ID, insn_idx, states_equal, env, &sl->state, cur, loop ? RANGE_WITHIN : NOT_EXACT)) {
hit:
			sl->hit_cnt++;
			...
			err = err ? : BPF_PROFILE_CALL_ARG(BPF_PROFILE_STATES_FILE_ID, insn_idx, propagate_precision, env, &sl->state, cur, NULL);
			if (err)
				return err;
