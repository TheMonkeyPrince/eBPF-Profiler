...
static int do_check_common(struct bpf_verifier_env *env, int subprog)
{
	...
	ret = CALL_WITH_BPF_TIMER(do_check, env);
out:
	if (!ret && pop_log)
...
int bpf_check(struct bpf_prog **prog, union bpf_attr *attr, bpfptr_t uattr, __u32 uattr_size)
{
	...
	ret = CALL_WITH_BPF_TIMER(check_cfg, env);
	if (ret < 0)
		goto skip_full_check;

	ret = CALL_WITH_BPF_TIMER(compute_postorder, env);
	if (ret < 0)
		goto skip_full_check;

	ret = CALL_WITH_BPF_TIMER(bpf_stack_liveness_init, env);
	if (ret)
		goto skip_full_check;

	ret = CALL_WITH_BPF_TIMER(check_attach_btf_id, env);
	if (ret)
		goto skip_full_check;

	ret = CALL_WITH_BPF_TIMER(compute_scc, env);
	if (ret < 0)
		goto skip_full_check;

	ret = CALL_WITH_BPF_TIMER(compute_live_registers, env);
	if (ret < 0)
		goto skip_full_check;

	ret = CALL_WITH_BPF_TIMER(mark_fastcall_patterns, env);
	if (ret < 0)
		goto skip_full_check;

	ret = CALL_WITH_BPF_TIMER(do_check_main, env);
	ret = ret ?: CALL_WITH_BPF_TIMER(do_check_subprogs, env);

	if (ret == 0 && CALL_WITH_BPF_TIMER(bpf_prog_is_offloaded, env->prog->aux))
		ret = CALL_WITH_BPF_TIMER(bpf_prog_offload_finalize, env);
